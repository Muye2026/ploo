...
For a `high` or `destructive` Operation Card, the one-call decision scope binds `operation:<material_digest>` alongside run, step, call, attempt, parameter digest, and capability choice. The full scope list and material-digest definition live in [workflow-state-schema.md](workflow-state-schema.md) (Operation Card integrity); the binding exists so an approval cannot survive a material card rewrite.
